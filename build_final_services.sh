...
# 🚀 Medical OCR Pipeline - Final 3 Services Builder
# Run this script once Docker filesystem is resolved

echo "🚀 Building Final 3 OCR Services (11-13/13)"
echo "============================================="
echo ""
...
if timeout 300 docker build -f docker/Dockerfile.doctr -t medical-ocr-pipeline-mcp-doctr . >/dev/null 2>&1; then
    ...
    echo "   ❌ Build failed - may need additional system packages"
fi
echo ""
...
if timeout 300 docker build -f docker/Dockerfile.qwen -t medical-ocr-pipeline-mcp-qwen . >/dev/null 2>&1; then
    ...
    echo "   ❌ Build failed - check PyTorch dependencies"
fi
echo ""
...
echo "======================"
echo "✅ Successfully built: $success/$total services"
echo "📊 Previous services: 10/13 (already operational)"
echo "🏆 Total completion: $((10 + success))/13 services ($(( (10 + success) * 100 / 13 ))%)"
echo ""

if [ $success -eq $total ]; then
    echo "🎉 MISSION COMPLETE! All 13 OCR services are now operational!"
    echo ""
    ...
    echo "🎯 Partial Success: $success additional services operational"
    echo "📊 Total: $((10 + success))/13 services working"
    echo ""
    echo "🔧 Troubleshoot remaining $(( total - success )) service(s):"
